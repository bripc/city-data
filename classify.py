...
# loop through all of the Tree objects to calculate statistics about native vs. non-native trees
# write this information to a CSV for easy portability in future projects
def classify_as_native(urban_forest, ca_native_flora):
    ...
    with open(config.LOCAL_PATH + '/output/classified_trees.csv', 'w') as csvfile:
        ...
        for tree in urban_forest:
            # there is some bogus data from the API that has no common or scientific name, ignoring these
